package com.nazarkopelchak.run.presentation.active_run.service
...
class ActiveRunService: Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(com.nazarkopelchak.core.presentation.designsystem.R.drawable.logo)
        ...
        when(intent?.action) {
            ACTION_START -> {
                val activityClass = intent.getStringExtra(EXTRA_ACTIVITY_CLASS)
                    ?: throw IllegalArgumentException("Activity class not provided")
